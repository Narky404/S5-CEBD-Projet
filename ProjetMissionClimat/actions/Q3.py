...
class Window(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        """On a repris le code de F2. Modifier l'interface pour proposer un choix de la région sans "
        ...
        #on fait la requete pour avoir toutes regions meme si on modif avant.
        cur = db.data.cursor()
        test = cur.execute("""
            SELECT nom_region
            FROM Regions"""
        )
        # on recup le resultat de la requete
        rows = cur.fetchall()
        liste_dep = []
        for i in range(len(rows)): #colonne
            for j in range(len(rows[0])): #ligne
                liste_dep.append(rows[i][j]) #on ajout tout les noms de regions dans une liste
        self.input['values']=liste_dep #la on ajout la liste pour le menu défilant
